...
<#assign shadowSize = "this.shadowSize = " + data.modelShadowSize + "F;">
<#if data.isBuiltInModel()>

@OnlyIn(Dist.CLIENT)
public class ${name}Renderer extends GeoEntityRenderer<${name}Entity.CustomEntity> {
@Override public ResourceLocation getEntityTexture(${name}Entity.CustomEntity entity) { 
return new ResourceLocation("${modid}:textures/entities/${data.mobModelTexture}");
 }
  public ${name}Renderer(EntityRendererManager renderManager) {
  ...
  @Override
  public void renderEarly(${name}Entity.CustomEntity animatable, MatrixStack stackIn, float ticks, IRenderTypeBuffer renderTypeBuffer, IVertexBuilder   vertexBuilder, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float partialTicks) {super.renderEarly(animatable, stackIn, ticks,   renderTypeBuffer, vertexBuilder, packedLightIn, packedOverlayIn, red, green, blue, partialTicks);
  }

  @Override
  public RenderType getRenderType(${name}Entity.CustomEntity animatable, float partialTicks, MatrixStack stack, IRenderTypeBuffer renderTypeBuffer,
  ...
  @Override
  protected float getDeathMaxRotation(${name}Entity.CustomEntity entity) {
            return 0.0F;
 }
  </#if>
}

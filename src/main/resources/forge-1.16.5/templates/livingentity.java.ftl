...
@${JavaModName}Elements.ModElement.Tag public class ${name}Entity extends ${JavaModName}Elements.ModElement {
	...
	public ${name}Entity(${JavaModName}Elements instance) {
		...
		FMLJavaModLoadingContext.get().getModEventBus().register(new ${name}Renderer.ModelRegisterHandler());
                ...
                FMLJavaModLoadingContext.get().getModEventBus().register(new ${name}GeckolibEntity());
                <#else>
                FMLJavaModLoadingContext.get().getModEventBus().register(new ${name}Renderer.ModelRegisterHandler());
                </#if>
                </#if>
	...
	public static class CustomEntity extends ${extendsClass}Entity <#if data.ranged && !settings.getMCreatorDependencies().contains("geckolib")>implements IRangedAttackMob</#if> <#if data.ranged && settings.getMCreatorDependencies().contains("geckolib")>implements IRangedAttackMob, IAnimatable</#if> <#if !data.ranged && settings.getMCreatorDependencies().contains("geckolib")>implements IAnimatable</#if> {
   <#if settings.getMCreatorDependencies().contains("geckolib")>
